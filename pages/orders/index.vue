  ...
  <el-card class="order-card">
    <div>
      ...
      <BaseFilter
        ...
        @reset="handleReset"
      />
      <BaseTable :goods="ordersList" :columns="columns">
...
async function fetchCustomers() {
  ...
  customerOptions.value = (res.data.data || []).filter((item: any) => item.status === '正常')
}
async function fetchGoods() {
  const res = await getGoods()
  goodsOptions.value = res.data.data || res
}
...
async function fetchOrders() {
  ...
  ordersList.value = (res.data.data || res).map((item: any) => ({
    ...item,
